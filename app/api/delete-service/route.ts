...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isDevelopment) {
      ...
      if (session) {
        console.log(`[DEV] Delete service by: ${session.user?.name || session.user?.email}`)
      }
    }
    ...
    if (isDevelopment) {
      ...
      const workspaceRoot = appDir.includes('/apps/admin') 
        ? join(appDir, '../..') 
        : appDir
      const filePath = join(workspaceRoot, `apps/site/src/content/services/${slug}.md`)
      
      // Also try .mdx extension for backwards compatibility
      const filePathMdx = join(workspaceRoot, `apps/site/src/content/services/${slug}.mdx`)
