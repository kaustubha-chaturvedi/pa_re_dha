...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isDevelopment()) {
      ...
      const possiblePaths = [
        `${CONTENT_PATHS.portfolio}/${slug}.md`,
        `${CONTENT_PATHS.portfolio}/${slug}.mdx`,
      ]

      let deleted = false
      for (const filePath of possiblePaths) {
        try {
          await deleteFile(filePath, `Delete portfolio case study: ${slug}`)
          deleted = true
          break
        } catch (error) {
          // Try next path
    ...
    const filePath = join(workspaceRoot, `${CONTENT_PATHS.portfolio}/${slug}.md`)
    const filePathMdx = join(workspaceRoot, `${CONTENT_PATHS.portfolio}/${slug}.mdx`)
